...
public class UserEntityServiceImpl implements UserEntityService {
    ...
    @Override
    public Mono<Object> save(DTOUserEntityRequest userEntityRequest) {
        return userEntityRepository.findByEmail(userEntityRequest.getEmail())
                ...
                .onErrorMap(InvalidUserException.class,
                        ex -> new UserEntityNotCreatedException(ex.getMessage()));
    }
    ...
    public Mono<Void> deleteById(Long id) {
        return userEntityRepository.findById(id)
                .switchIfEmpty(Mono.error(new UserEntityNotFoundException("User with id " + id + " not found")))
                .flatMap(userEntity -> {
                    userEntityRepository.delete(userEntity).subscribe();
                    return Mono.empty();
                });
    }
    ...
    private Mono<DTOUserEntityResponse> updatedMethod(Long id, DTOUserEntityRequest userEntity) {
        return userValidator.validateDto(userEntity)
                .then(Mono.defer(() -> userEntityRepository.findById(id)
                        ...
                        .onErrorMap(DuplicateKeyException.class,
                                ex -> new UserEntityNotCreatedException("User not updated due to duplicate email"))));
    }

    private Mono<DTOUserEntityResponse> saveMethod(DTOUserEntityRequest userEntity) {
    return userValidator.validateDto(userEntity)
        .then(Mono.defer(() -> {
            UserEntity entity = entityMapper.dtoRequestToEntity(userEntity);
            return userEntityRepository.save(entity)
                .map(entityMapper::entityToDtoResponse)
                .switchIfEmpty(Mono.error(new UserEntityNotCreatedException("User not saved")));
        }))
        .onErrorMap(DuplicateKeyException.class,
            ex -> new UserEntityNotCreatedException("User not saved due to duplicate email"));
}
